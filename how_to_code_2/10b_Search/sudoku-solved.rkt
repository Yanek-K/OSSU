...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname sudoku-solved) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (valid-board? bd)
  (local [(define (valid-units? lou)        ; (listofUnits) -> Boolean
          ...
          (define (read-pos p)              ; Pos -> Val|False    -> Produce contents of board at P
            (read-square bd p))
          
          (define (keep-only-values lovf)   ; (list of Val|False) -> (list of Val)
            (filter number? lovf))

          (define (no-duplicates? lov)        ; (list of Val) -> Boolean  -> Produce true if no value in lov appears twise
            (cond [(empty? lov) true]
                  [else
